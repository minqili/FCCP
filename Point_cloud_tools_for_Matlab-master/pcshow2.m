...
[X, Y, Z, C, markerSize, vertAxis, vertAxisDir, currentAxes, map] = ...
...
if isempty(C)
    scatter3(currentAxes, X, Y, Z, markerSize, Z, '.', 'Tag', 'pcviewer');
elseif (ischar(C) || isequal(size(C),[1,3]))
    try
        plot3(currentAxes, X, Y, Z, '.', 'Color', C, 'MarkerSize', sqrt(markerSize), 'Tag', 'pcviewer');
        grid(currentAxes, 'on');
    catch exception
        throwAsCaller(exception);
    end
else
    scatter3(currentAxes, X, Y, Z, markerSize, C, '.', 'Tag', 'pcviewer');
end
...
%========================================================================== 
function [X, Y, Z, C, markerSize, vertAxis, vertAxisDir, ax, map] = validateAndParseInputs(varargin)
...
if  ~bitget(nargin, 1) && ~isa(varargin{1}, 'pointCloud')
    [X, Y, Z, C, map] = vision.internal.pc.validateAndParseInputsXYZC(mfilename, varargin{1:2});
    pvpairs = varargin(3:end);
else
    [X, Y, Z, C, map] = vision.internal.pc.validateAndParseInputsXYZC(mfilename, varargin{1});
